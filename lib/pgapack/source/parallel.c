	      ...
	      MPI_Comm comm)
{
    ...
    /* Added an extra communicator for the lammps calculations */
    int color;
    color = rank;
    MPI_Comm lammps_comm;
    MPI_Comm_split (comm,color,rank,&lammps_comm);

    PGAEvaluate(ctx, PGA_OLDPOP, f, comm, lammps_comm);
    if (rank == 0)
	...
	PGAEvaluate(ctx, PGA_NEWPOP, f, comm, lammps_comm);
	if (rank == 0)
...
void PGAEvaluateSeq(PGAContext *ctx, int pop,
		    double (*f)(PGAContext *, int, int, MPI_Comm))
{
    int     p;
    double  e;
    MPI_Comm comm;
...
void PGAEvaluateCoop(PGAContext *ctx, int pop,
		     double (*f)(PGAContext *, int, int, MPI_Comm), MPI_Comm comm, MPI_Comm slave)
{
    MPI_Status      stat;
    int             p, fp, q;
    double          e;
	...
	if (p<ctx->ga.PopSize) {
	    ...
	    PGASetEvaluation(ctx, p, pop, e);
#if DEBUG_EVAL 
// NASA Ames... 1) validation of code, CFD nra - open foam - ablation code - tato and 2) UQ
...
void PGAEvaluateMS(PGAContext *ctx, int pop,
		   double (*f)(PGAContext *c, int p, int pop, MPI_Comm), MPI_Comm comm)
{
    int    *work;
    int     i, k, s, p, size, sentout, rank;
    double  e;
    MPI_Status stat;
    PGAIndividual *ind;
...
/*I****************************************************************************
   ...
   Inputs:  Added extra communicator for lammps calculations
...
void PGAEvaluateSlave(PGAContext *ctx, int pop,
		      double (*f)(PGAContext *, int, int, MPI_Comm), MPI_Comm comm, MPI_Comm slave_comm)
{
    MPI_Status  stat;
    int         k;
    double      e;
	...
	e = (*f)(ctx, PGA_TEMP1, pop, slave_comm);
		 ...
		 double (*f)(PGAContext *, int, int, MPI_Comm), MPI_Comm comm, MPI_Comm slave_comm)
{
    ...
    /* Added slave communicator for lammps calculations on a single proc */
    int color;
    color = rank;
    MPI_Comm_split (comm,color,rank,&slave_comm);

    if (rank == 0) {
	if (size == 1)
	    PGAEvaluateSeq(ctx, pop, f);
	if (size == 2)
	    PGAEvaluateCoop(ctx, pop, f, comm, slave_comm);
	if (size > 2) 
	    PGAEvaluateMS(ctx, pop, f, comm);
    } else {
	PGAEvaluateSlave(ctx, pop, f, comm, slave_comm);
    }
