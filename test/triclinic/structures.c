...
void geo2data (char *sfile, char ***dfile, char ***atypes, int **cellflag, char ***fname,
	       ffieldtype *fftype, int *num, int forcefield2optimize, int rank)
{
  FILE *fi, *fo;
  ...
  if (fi == NULL) {
    if (rank==0) fprintf (stderr, "ERROR: opening the geo file %s terminating...\n", sfile);
  ...
  if ((*num) == 0) {
    if (rank==0) printf ("ERROR: No valid structures read from %s!\n", sfile);
    MPI_Abort (MPI_COMM_WORLD, 1);
  }
  fclose (fi);
...
//    rstrain[i] = (restraint *) scalloc (1, sizeof (restraint), "rsti");
  ...
  if ((fi = fopen (filen, "r")) == NULL) {
    if (rank==0) fprintf (stderr, "ERROR: opening structure file %s -- terminating !\n",sfile);
    MPI_Abort (MPI_COMM_WORLD, 1);
  }
  i = -1;
  while (fgets (line, MAX_LINE, fi) != NULL) {
	...
	strcpy ((*fname)[i], header);
      ...
      i++;
      if (rank == 0 && debug_level == 1) DEBUG_PRINT ((">> Reading structure %d of %d\n", (i + 1), *num));
      j = n = 0;
      has_cell = 0;
	...
	if (rank==0) printf ("ERROR: Cannot create LAMMPS data file %s!\n", fout);
	MPI_Abort (MPI_COMM_WORLD, 1);
      }
      fprintf (fo, "Atoms\n\n");
    }
    else if (i > -1 && (strstr (line, "CRYSTX"))) {
      k =sscanf (line, "%s %lf %lf %lf %lf %lf %lf", junk, &cell[0], &cell[1], &cell[2], &cell[3],
		&cell[4], &cell[5]);
      if (k == 7 && cell[0] > 0.0 && cell[1] > 0.0 && cell[2] > 0.0 && cell[3] > 0.0
	  && cell[4] > 0.0 && cell[5] > 0.0)
	has_cell = 1;
    }
    else if (i > -1 && (strstr (line, "Lattice"))) {
      stok = strtok (line, "Lattice=\"");
      k =sscanf (stok, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &cell[0], &cell[1], &cell[2], &cell[3],
                &cell[4], &cell[5], &cell[6],  &cell[7],  &cell[8]);
      if (k == 9)
        has_cell = 1;

      init_cell (cell, i, rank, file_format);
    }
	...
	rstrain[current_rst].f2 = atof (strtmp[6]);
        if (rank == 0 && debug_level == 1) DEBUG_PRINT ((">> Bond restraint on %s: atoms %d %d %4.2f %4.2f %4.2f\n", header,
        ...
        if (rank == 0 && debug_level == 1) DEBUG_PRINT ((">> Transition State restraint on %s: atoms %d %d %d %4.2f %4.2f %4.2f\n", rstrain[current_rst].name,
          rstrain[current_rst].atom[0], rstrain[current_rst].atom[1],
	...
	rstrain[current_rst].f1 = atof (strtmp[6]);
	rstrain[current_rst].f2 = atof (strtmp[7]);
        if (rank == 0 && debug_level == 1) DEBUG_PRINT ((">> Angle restraint on %s: atoms %d %d %d %4.2f %4.2f %4.2f\n", header,
          rstrain[current_rst].atom[0], rstrain[current_rst].atom[1],
                rstrain[current_rst].atom[2], rstrain[current_rst].val, rstrain[current_rst].f1,
                rstrain[current_rst].f2));
      }
	...
	rstrain[current_rst].f2 = atof (strtmp[8]);
        if (rank == 0 && debug_level == 1) DEBUG_PRINT ((">> Torsion restraint on %s: atoms %d %d %d %d %4.2f %4.2f %4.2f\n",header,
                rstrain[current_rst].atom[0], rstrain[current_rst].atom[1],
                rstrain[current_rst].atom[2], rstrain[current_rst].atom[3],
                rstrain[current_rst].val, rstrain[current_rst].f1, rstrain[current_rst].f2));
      }
      else {
	if (rank==0) printf ("ERROR: Unkown RESTRAINT in %s.\n\n", rstrain[current_rst].name);
	MPI_Abort (MPI_COMM_WORLD, 1);
      }
      current_rst++;
    }
...
//      fstr = strstr (typestr, junk);
	...
	sprintf (junk, "%d %lf\n", c, amass);
	strcat (massstr, junk);
        if (forcefield2optimize == 0) sprintf (junk, "%d ", (atype_id + 1));
	else sprintf (junk, "%s ", type_id);
...
//        printf("atomtype=%d fstr=%d [%s] typestr=%d [%s] c=%d\n", atype_id, fstr, fstr, typestr, typestr, c);
      ...
      if (forcefield2optimize == REAX || forcefield2optimize == REAXC || forcefield2optimize == COMB) {
       if (!pqeq_flag) fprintf (fo, "%d %d %lf %lf %lf %lf\n", j, c, 0.00, x, y, z);
       else fprintf (fo, "%d %d %lf %lf %lf %lf 0.0 0.0 0.0\n", j, c, -1.00, x, y, z);
      }
      else if (forcefield2optimize == MORSE || (forcefield2optimize == TERSOFF) || (forcefield2optimize == TERSOFF_MOD) || (forcefield2optimize == ZHOU_EAM))
...
//        printf("new type: c=%d\n", c);
      ...
      if (forcefield2optimize == REAX || forcefield2optimize == REAXC || forcefield2optimize == COMB) {
//modified by BK
       if (!pqeq_flag) fprintf (fo, "%d %d %lf %lf %lf %lf\n", j, c, 0.00, x*Rot[0][0]+y*Rot[1][0]+z*Rot[2][0], x*Rot[0][1]+y*Rot[1][1]+z*Rot[2][1], x*Rot[0][2]+y*Rot[1][2]+z*Rot[2][2]);
       //if (!pqeq_flag) fprintf (fo, "%d %d %lf %lf %lf %lf\n", j, c, 0.00, x, y, z);
       else fprintf (fo, "%d %d %lf %lf %lf %lf 0.0 0.0 0.0\n", j, c, -1.00, x, y, z);
      }
      else if (forcefield2optimize == MORSE || (forcefield2optimize == TERSOFF) || (forcefield2optimize == TERSOFF_MOD) || (forcefield2optimize == ZHOU_EAM))
...
void init_cell (double *cell, int i, int rank, int file_format)
{
...
//      a = A;
//      b = B * sin_gamma;
//      c = sqrt (sq_c * sin_beta * sin_beta - C * (cos_alpha - cos_gamma * cos_beta) / sin_gamma);
//      xy = B * cos_gamma;
//      xz = C * cos_beta;
//      yz = C * (cos_alpha - cos_gamma * cos_beta) / sin_gamma;
//////////
      ...
      if (fabs (prod) < 1.) 
        ...
        if (rank==0) printf("Error: scalar product alpha %f\n", prod);
        MPI_Abort (MPI_COMM_WORLD, 1);
      }
      ...
      if (fabs (prod) < 1.)
        ...
        if (rank==0) printf("Error: scalar product beta %f\n", prod);
        MPI_Abort (MPI_COMM_WORLD, 1);
      }
      ...
      if (fabs (prod) < 1.)
        ...
        if (rank==0) printf("Error: scalar product gamma %f\n", prod);
        MPI_Abort (MPI_COMM_WORLD, 1);
      }
      sq_c = C * C;
      cos_alpha = cos (alpha * PI / 180.0);
...
//    Rotation!!!!!!!!
      ...
      for(n=0;n<3;n++){
          for(m=0;m<3;m++)
              bb[n][m]=((aa[(m+1)%3][(n+1)%3] * aa[(m+2)%3][(n+2)%3]) - (aa[(m+1)%3][(n+2)%3]*aa[(m+2)%3][(n+1)%3]))/ determinant;
      }
...
void write_fdata (char *header, char *masses, double *bounds, int cell_flag, double *cell,
		  int ntype, int natom, int i, int rank, int file_format)
{
  ...
  if ((fo = fopen (fout, "w")) == NULL) {
    if (rank==0) fprintf (stderr, "ERROR: Writing to %s! terminating...\n", fout);
    MPI_Abort (MPI_COMM_WORLD, 1);
  }
  sprintf (fout, "_tmp.dat.%d",rank);
  if ((fi = fopen (fout, "r")) == NULL) {
    if (rank==0) printf ("ERROR: Cannot read from temporary file file %s!\n", fout);
    MPI_Abort (MPI_COMM_WORLD, 1);
  }
  ...
  if (!cell_flag) {
    fprintf (fo, "%lf %lf xlo xhi\n%lf %lf ylo yhi\n%lf %lf zlo zhi\n\n", bounds[0] - 100,
	     bounds[1] + 100, bounds[2] - 100, bounds[3] + 100, bounds[4] - 100, bounds[5] + 100);
//    fprintf (fo, "%lf %lf xlo xhi\n%lf %lf ylo yhi\n%lf %lf zlo zhi\n\n", bounds[0],
...
*  Parameters   : Atom type LAMMPS id, reax/morse/comb force field data structure (ffdata), and
                  MPI process rank
